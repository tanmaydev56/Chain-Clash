...
export class ChainClashRoom extends DurableObject<RealtimeEnv> {
  ...
  async fetch(request: Request) {
    ...
    const seat = await this.env.DB.prepare('SELECT id,name,is_bot,score,lives,shield,joined_at FROM players WHERE room_code=? AND user_id=?').bind(code, ticket.userId).first<{ id: string; name: string; is_bot: number; score: number; lives: number; shield: number; joined_at: number }>();
    ...
    return this.commandQueue.run(async () => {
      ...
      if (!snapshot.players.some((player) => player.id === seat.id)) {
        ...
        snapshot.players.push({ id: seat.id, userId: ticket.userId, name: seat.name, bot: Boolean(seat.is_bot), score: seat.score, lives: seat.lives, shield: Boolean(seat.shield), joinedAt: seat.joined_at, disconnectedAt: null });
  ...
  private async ensureSnapshot(code: string) {
    ...
    const room = await this.env.DB.prepare('SELECT code,host_player_id,category,mode,blocked_letter,status,current_letter,turn_player_id,winner_player_id,turn_deadline,challenge_key,state_version,updated_at FROM rooms WHERE code=?').bind(code).first<{ code: string; host_player_id: string; category: Category; mode: string; blocked_letter: string | null; status: 'waiting' | 'active' | 'finished'; current_letter: string; turn_player_id: string | null; winner_player_id: string | null; turn_deadline: number | null; challenge_key: string | null; state_version: number; updated_at: number }>();
  ...
  private async save(persistD1: boolean) {
    ...
    for (const p of state.players) statements.push(this.env.DB.prepare(`INSERT INTO players (id,user_id,room_code,name,is_bot,score,lives,shield,joined_at,last_seen_at) VALUES (?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET name=excluded.name,score=excluded.score,lives=excluded.lives,shield=excluded.shield,last_seen_at=excluded.last_seen_at`).bind(p.id,p.userId,state.code,p.name,p.bot?1:0,p.score,p.lives,p.shield?1:0,p.joinedAt,state.updatedAt));
    statements.push(this.env.DB.prepare('UPDATE rooms SET category=?,blocked_letter=?,host_player_id=?,status=?,current_letter=?,turn_player_id=?,winner_player_id=?,turn_deadline=?,state_version=?,updated_at=? WHERE code=?').bind(state.category,state.blockedLetter,state.hostPlayerId,state.status,state.currentLetter,state.turnPlayerId,state.winnerPlayerId,state.deadline,state.version,state.updatedAt,state.code));
