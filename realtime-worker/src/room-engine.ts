...
export type RealtimePlayer = { id: string; userId: string | null; name: string; bot: boolean; score: number; lives: number; joinedAt: number; disconnectedAt: number | null };
...
export type RealtimeRoomState = {
  code: string; hostPlayerId: string; category: Category; status: 'waiting' | 'active' | 'finished'; currentLetter: string;
...
function advance(state: RealtimeRoomState, currentPlayerId: string, now: number, nextLetter = state.currentLetter) {
  ...
  state.deadline = next ? now + (next.bot ? ROOM_POLICY.botDelayMs : ROOM_POLICY.turnMs) : null;
}
...
export function submitWord(state: RealtimeRoomState, userId: string, commandId: string, rawWord: string, valid: boolean, now: number): SubmitResult {
  ...
  if (acceptedWord) { player.score += word.length * 10; next.usedWords.push(word); }
...
export function resolveAlarm(state: RealtimeRoomState, now: number): RealtimeRoomState {
  ...
  if (player.bot) {
    ...
    if (options.length) {
      const word = options[0]; player.score += word.length * 10; next.usedWords.push(word);
...
export function addBot(state: RealtimeRoomState, userId: string, commandId: string, now: number): SubmitResult {
  ...
  const next = copy(state); const bot: RealtimePlayer = { id: crypto.randomUUID(), userId: null, name: 'WordBot', bot: true, score: 0, lives: 3, joinedAt: now, disconnectedAt: null };
  ...
  if (next.status === 'waiting') { next.status = 'active'; next.turnPlayerId = next.hostPlayerId; next.deadline = now + ROOM_POLICY.turnMs; }
...
export function publicRoomState(state: RealtimeRoomState) {
  return { room: { code: state.code, category: state.category, status: state.status, current_letter: state.currentLetter, turn_player_id: state.turnPlayerId, winner_player_id: state.winnerPlayerId, turn_deadline: state.deadline, state_version: state.version }, players: state.players.map(({ userId: _userId, disconnectedAt: _disconnectedAt, ...player }) => ({ id: player.id, name: player.name, is_bot: player.bot ? 1 : 0, score: player.score, lives: player.lives, joined_at: player.joinedAt })), moves: state.moves.slice(-12).reverse().map((move) => ({ id: move.id, word: move.word, valid: move.valid ? 1 : 0, created_at: move.createdAt, player_name: state.players.find((player) => player.id === move.playerId)?.name ?? 'Player' })) };
}
