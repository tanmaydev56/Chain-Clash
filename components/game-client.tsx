  ...
  return (
    <main className="min-h-dvh overflow-hidden bg-background text-foreground">
      ...
      <header className="relative z-10 mx-auto flex w-full max-w-7xl items-center justify-between px-5 py-5 sm:px-8">
        ...
        <div className="flex items-center gap-2"><span className="hidden items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-2 text-xs font-semibold text-muted-foreground sm:flex"><Globe2 className="size-3.5 text-primary" /> Live multiplayer</span>{/* Use document navigation until Vinext's RSC client navigation is reliable in production. */}<a href={profile ? '/profile' : '/signin'} className="hidden rounded-full border border-white/15 bg-white/5 px-3 py-2 text-xs font-black hover:bg-white/10 sm:block">{profile ? profile.googleLinked ? 'Profile' : 'Save progress' : 'Sign in'}</a>{profile?.googleLinked && <Button aria-label="Sign out" onClick={() => void logout()} variant="ghost" size="icon-sm" className="rounded-full border border-white/10 bg-white/5 text-muted-foreground"><LogOut /></Button>}<Button aria-label={soundOn ? 'Mute game sounds' : 'Enable game sounds'} onClick={() => { const next = !soundOn; setSoundOn(next); window.localStorage.setItem('chain-clash-sound', next ? 'on' : 'off'); if (next) playTone(true, 520); }} variant="ghost" size="icon-sm" className="rounded-full border border-white/10 bg-white/5 text-muted-foreground">{soundOn ? <Volume2 /> : <VolumeX />}</Button></div>
      </header>
      ...
      {/* oxlint-disable-next-line next/no-html-link-for-pages -- Vinext RSC navigation intermittently aborts in production; normal navigation is the reliable fallback. */}
      <nav className="relative z-10 mx-auto flex w-full max-w-7xl flex-wrap gap-4 px-5 pb-10 text-sm font-bold text-primary sm:px-8"><a href="/profile">Profile</a><a href="/leaderboard">Leaderboard</a><a href="/modes">Modes</a><a href="/how-to-play">How to play</a></nav>
